...
import ch.diso.ex21_Lambdas.FeatureMatchers;
...
/**
 * Exercise - Filtering and Collecting
 * <p>
...
public class ElectoralDistrictTest {
    ...
    public void setOfVotersInDistrict() {
        ...
        Set<RegisteredVoter> votersInHackney =
                ElectoralDistrict.votersIn(ElectoralDistrict.HACKNEY, allVoters);

        assertThat(votersInHackney, hasSize(2));
        assertThat(votersInHackney, containsInAnyOrder(
                aVoterWithId("HA7654"),
                aVoterWithId("HA2213")));
    }
    ...
    private static Matcher<Person> aPersonNamed(String name) {
        return FeatureMatchers.from(
                is(name), "a person", "name", Person::getName);
    }

    private static Matcher<RegisteredVoter> aVoterWithId(String name) {
        return FeatureMatchers.from(
                is(name), "a voter", "electorId", RegisteredVoter::getElectorId);
    }

    private static Matcher<Ballot> spoiled() {
        return FeatureMatchers.from(
                equalTo(Boolean.TRUE), "a spoiled ballot", "isSpoiled", Ballot::isSpoiled);
    }
